...
// temporary castle basic-auth proxy
if (env.CASTLE_HOST) {
  app.use('/castle', proxy(env.CASTLE_HOST, {
    proxyReqOptDecorator: (proxyReqOpts, srcReq) => {
      if (env.CASTLE_AUTH_USER && env.CASTLE_AUTH_PASS) {
        let auth = new Buffer(`${env.CASTLE_AUTH_USER}:${env.CASTLE_AUTH_PASS}`).toString('base64');
