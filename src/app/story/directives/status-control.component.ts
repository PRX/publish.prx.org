  ...
  template: `
    ...
    <div class="actions" *ngIf="story">
      <prx-button
        ...
        disabled="0"
        plain="1"
        [visible]="isChanged || nextStatus !== currentStatus"
        (click)="discard()"
        >Discard</prx-button
      >
      ...
      >
        ...
        <div *ngIf="buttons.secondary" class="dropdown-menu-items">
          <button *ngIf="id" class="secondary" (click)="buttons.secondary.action($event)">
            ...
            <prx-icon [name]="buttons.secondary.icon" color="red" size="1em"></prx-icon>
          </button>
        </div>
      </prx-button>
    </div>
...
export class StatusControlComponent implements DoCheck {
  ...
  constructor(private modal: ModalService, private toastr: ToastrService, private router: Router, private angulartics2: Angulartics2) {}
  ...
  showProblems() {
    ...
    this.modal.show({
      title,
      body: `<ul>${errors.join('')}</ul>`,
      secondaryButton: 'Okay'
    });
  }

  private storyInvalid(strict): string {
    const invalids = this.story.invalid(null, strict);
